...
- (void)verifyAuthorizationWithAccessCode:(NSString *)accessCode;
{
  @synchronized(self) {
    ...
    [requests addObject:request];
     ...
     /* ASIHTTPRequest *request = [ASIHTTPRequest requestWithURL:self.tokenURL];
      [request setPostValue:@"fcc9c5bf6f464b5fb1e5d084b64b54e4" forKey:@"client_id"];
      [request setPostValue:@"cf311b8c62634aeb8ec23f7274162acf" forKey:@"client_secret"];
      [request setPostValue:@"authorization_code" forKey:@"grant_type"];
      [request setPostValue:@"http://www.google.com" forKey:@"redirect_uri"];
...
- (void)request:(ASIHTTPRequest *)request didReceiveData:(NSData *)rawData
{
  ...
  if( [request isResponseCompressed]) {
    //data = [ASIHTTPRequest uncompressZippedData:rawData];
      data=[ASIDataDecompressor uncompressData:rawData error:&error];
      NSLog(@"DATA IS COMPRESSED CHECK LROAUTH2CLIENT.m Line 161");
  }
